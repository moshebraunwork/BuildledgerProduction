...
import { Download, Send, Eye, Pencil, Printer, Plus, Trash2, Mail, X } from "lucide-react";
...
export function InvoicesManager({
  ...
  async function downloadPdf(inv: Invoice) {
    ...
    items.forEach((l, idx) => {
      ...
      if (l.qty != null) {
        doc.setTextColor(...gray);
        doc.text(String(l.qty), margin + 120, y + 5.5, { align: "right" });
      }
      doc.setTextColor(...dark);
      doc.text(fmtMoney(l.amount), pageW - margin - 3, y + 5.5, { align: "right" });
      y += 9;
    });
    ...
    doc.setFontSize(10);
    doc.setTextColor(...gray);
    doc.text("Subtotal", pageW - margin - 45, y);
    doc.setTextColor(...dark);
    doc.text(fmtMoney(inv.subtotal), pageW - margin, y, { align: "right" });
    y += 7;

    doc.setTextColor(...gray);
    doc.text("Tax", pageW - margin - 45, y);
    doc.setTextColor(...dark);
    doc.text(fmtMoney(inv.tax), pageW - margin, y, { align: "right" });
    y += 5;

    // Total highlight
    doc.setFillColor(...blue);
    doc.rect(pageW - margin - 60, y, 60, 11, "F");
    doc.setFontSize(11);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(...white);
...
.inv-label{font-size:22px;font-weight:700;letter-spacing:3px}
.inv-num{font-size:12px;color:#bfdbfe;margin-top:2px;text-align:right}
...
.bill-to .email{font-size:12px;color:#64748b}
...
table.items td{padding:8px 12px;border-bottom:1px solid #e2e8f0;font-size:13px}
  ...
  return (
    <>
              ...
              {invoices.map((inv) => (
                <TableRow key={inv.id}>
                  ...
                  <TableCell className="text-right whitespace-nowrap">
                    <Button size="sm" variant="ghost" title="Preview" onClick={() => openViewing(inv)}>
      ...
      <Dialog
        open={!!viewing}
        onOpenChange={(o) => {
          if (!o) { setViewing(null); setEditing(false); setDraft(null); }
        }}
      >
              ...
              <div className="space-y-6 mt-2">
                  ...
                  <div>
                    {company?.logo_url && (
                      <img
                        ...
                        alt="logo"
                        className="max-h-10 max-w-[160px] object-contain mb-2"
                      />
                    )}
                    ...
                    {editing ? (
                      <div className="space-y-2">
                        <Input
                          ...
                          onChange={(e) => setDraft((d) => d ? { ...d, customer_name: e.target.value } : d)}
                          placeholder="Customer name"
                        />
                        <Input
                          type="email"
                          value={draft?.customer_email ?? ""}
                          onChange={(e) => setDraft((d) => d ? { ...d, customer_email: e.target.value } : d)}
                ...
                <div>
                          ...
                          <TableCell className="text-right">
                            {editing ? (
                              <Input
                                ...
                                onChange={(e) => {
                                  const items = [...(draft?.line_items ?? [])];
                                  items[i] = { ...items[i], qty: e.target.value ? Number(e.target.value) : undefined };
                                  recalcDraft(items);
                                }}
                          ...
                          {editing && (
                            <TableCell>
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => recalcDraft((draft?.line_items ?? []).filter((_, idx) => idx !== i))}
                              >
                  ...
                  {editing && (
                    <Button
                      ...
                      variant="outline"
                      className="mt-2"
                      onClick={() => recalcDraft([...(draft?.line_items ?? []), { name: "", amount: 0 }])}
                    >
                ...
                <div className="ml-auto w-60 space-y-1.5 text-sm">
                  <div className="flex justify-between text-muted-foreground">
                    <span>Subtotal</span><span>{fmtMoney(current.subtotal)}</span>
                  </div>
                  <div className="flex justify-between text-muted-foreground">
                    <span>Tax</span><span>{fmtMoney(current.tax)}</span>
                  </div>
                  <Separator />
                  <div className="flex justify-between font-bold text-base">
                    <span>Total</span><span>{fmtMoney(current.total)}</span>
                  </div>
                </div>
      ...
      <Dialog open={sendDialogOpen} onOpenChange={(o) => { if (!o) setSendDialogOpen(false); }}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Send {sendTarget?.number}</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <label className="flex items-center gap-2 cursor-pointer text-sm">
                  ...
                  />
                  Customer email
                  {sendTarget?.customer_email && (
                    <span className="text-muted-foreground">({sendTarget.customer_email})</span>
                  )}
                </label>
                <label className="flex items-center gap-2 cursor-pointer text-sm">
                  <input
